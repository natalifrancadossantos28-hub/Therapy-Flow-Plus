-- =========================================================================
-- 0087_list_appointments_ordenado.sql
--
-- A Visão Mensal (e o Dashboard) paravam de mostrar atendimentos no meio do
-- mês: o PostgREST devolve no máximo 1000 linhas por requisição e
-- list_appointments não tinha order by, então o corte caía num ponto
-- arbitrário do período. O cliente passa a paginar a leitura, o que exige uma
-- ordenação estável.
--
-- Só adiciona "order by date, time, id" — assinatura e colunas inalteradas.
-- Idempotente.
-- =========================================================================
...
begin
  ...
  return query
    ...
    -- Ordem estável: o cliente pagina a leitura (o PostgREST corta em 1000
    -- linhas por requisição) e sem order by as páginas se repetiriam/pulariam.
