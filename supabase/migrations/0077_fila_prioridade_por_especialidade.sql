-- =========================================================================
-- 0077_fila_prioridade_por_especialidade.sql
--
-- Regras de prioridade da FILA DE ESPERA por especialidade.
--
-- Antes: a prioridade por idade (bônus <4 anos = +50, 4–6 anos = +20 e
-- "Prioridade Máxima" para menores de 5) valia para TODAS as especialidades.
--
-- Agora, por especialidade:
--   • Fonoaudiologia e Fisioterapia .......... regra atual (prioridade máxima
--     por idade/abrigo + bônus de primeira infância).
--   • Nutrição e Psicologia Parental ......... FIFO puro: sem bônus de idade e
--     sem prioridade máxima; ordena estritamente pela data de entrada na fila.
--   • Psicologia, Psicomotricidade e T.O. .... prioridade por idade só a partir
--     de 3 anos (bebês de 0–2 anos não recebem bônus nem prioridade máxima por
--     idade; a vulnerabilidade "abrigo" continua valendo).
--   • Demais especialidades .................. regra atual, sem mudança.
--
...
--   'min3'  → prioridade por idade só para 3 anos ou mais
--   'full'  → regra padrão (prioridade desde o nascimento)
-- ═══════════════════════════════════════════════════════════════════════
...
as $$
  select case
    ...
    -- "Parental" antes de "psicolog%" porque "Psicologia Parental" casa nos dois.
...
-- ═══════════════════════════════════════════════════════════════════════
-- Step 3: prioridade gravada na entrada da fila respeita a política
-- ═══════════════════════════════════════════════════════════════════════
