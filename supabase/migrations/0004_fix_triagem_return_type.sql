-- Phase 3 hotfix: the list/get/upsert RPCs declared return type
-- public.triagem_row but "return query select _triagem_to_row(t)" produces a
-- single-column row (where column 1 is the whole composite), which PostgREST
-- rejects with 42804 "structure of query does not match function result type".
--
-- Fix: return the table type public.triagens directly. Column layout matches
-- what the frontend expects (mapRow already reads snake_case columns).
...
-- Grants unchanged (inherited from 0003), but re-issued for safety.
