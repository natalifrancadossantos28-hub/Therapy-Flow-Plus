-- 0042_patient_photo.sql
-- Foto do paciente: adiciona a coluna patients.photo_url (URL publica no
-- Supabase Storage), ensina o upsert_patient a gravar `photoUrl`, e cria o
-- bucket 'patient-photos' com policies para o app (anon) subir/ler as fotos.
--
-- A imagem e comprimida/redimensionada no navegador (~50-100kb) antes do
-- upload; aqui guardamos apenas o link, mantendo a tabela e as listagens leves.
...
-- 2) upsert_patient: passa a aceitar photoUrl (baseado no 0041) --------------
...
begin
  ...
  if p_id is null then
    insert into public.patients (
      ...
      tipo_registro, local_atendimento, photo_url
    ) values (
      v_company_id,
...
-- Policies: o app usa a chave anon (mesmo padrao das RPCs). Restrito ao bucket.
...
create policy "patient_photos_update" on storage.objects
  for update using (bucket_id = 'patient-photos') with check (bucket_id = 'patient-photos');
create policy "patient_photos_delete" on storage.objects
  for delete using (bucket_id = 'patient-photos');
