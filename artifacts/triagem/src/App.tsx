...
function Relatorio({ resultado, onNova }: { resultado: Resultado; onNova: () => void }) {
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="bg-primary text-primary-foreground py-8 px-6 text-center shadow-md">
        <h1 className="text-2xl md:text-3xl font-bold">Relatório de Triagem Multidisciplinar</h1>
        <p className="mt-1 text-primary-foreground/80 text-sm">NFs – Triagem Multidisciplinar</p>
      </div>
      ...
      <div className="max-w-4xl mx-auto p-4 md:p-8 space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-border p-6 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-4xl font-bold text-foreground mt-1">
              {totalPontos} <span className="text-lg font-normal text-muted-foreground">/ {totalMax}</span>
            </p>
        ...
        {/* Resultado por área (ranking completo) */}
        <div className="bg-white rounded-2xl border border-border p-6 shadow-sm">
            ...
            {ranking.map(({ area, pontos, max, pct, nivel }) => (
              <div key={area}>
                ...
                <div className="h-2.5 bg-secondary rounded-full overflow-hidden">
                  <div className={`h-full rounded-full transition-all duration-700 ${BARRA_AREA[area] ?? "bg-gray-500"}`}
                    style={{ width: `${pct}%` }} />
                </div>
              </div>
            ))}
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-2xl p-5 text-sm text-blue-900">
          ...
          <p>Este documento refere-se a uma triagem inicial e <strong>não constitui diagnóstico clínico</strong>. Recomenda-se avaliação por profissional habilitado.</p>
        </div>

        {/* Legenda */}
        <div className="bg-white rounded-2xl border border-border p-4 shadow-sm">
          <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-3">Legenda</p>
          <div className="flex flex-wrap gap-3 text-sm">
            {[
              { label: "Baixo indicativo", cor: "bg-emerald-100 text-emerald-800 border-emerald-200", desc: "≤ 10 pontos" },
              { label: "Indício moderado",  cor: "bg-amber-100 text-amber-800 border-amber-200",   desc: "11–20 pontos" },
              { label: "Indício elevado",   cor: "bg-rose-100 text-rose-800 border-rose-200",       desc: "> 20 pontos" },
            ].map((n) => (
              <div key={n.label} className={`px-4 py-2 rounded-xl border ${n.cor}`}>
                <span className="font-bold">{n.label}</span>
                <span className="ml-2 opacity-70">{n.desc}</span>
              </div>
            ))}
          </div>
        </div>
...
export default function App() {
  ...
  if (resultado) return <Relatorio resultado={resultado} onNova={() => setResultado(null)} />;
