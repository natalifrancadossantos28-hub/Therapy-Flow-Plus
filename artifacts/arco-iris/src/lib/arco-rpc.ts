...
function mapTriagemMulti(r: TriagemMultiRow): TriagemMulti {
  return {
    id: Number(r.id),
    nome: r.nome,
...
export type TriagemMultiInput = Partial<Omit<TriagemMulti, "id" | "createdAt" | "nome">> & { nome: string };

const onlyDigits = (s: string | null | undefined) => (s ?? "").replace(/\D/g, "");
const normName = (s: string | null | undefined) => (s ?? "").trim().toLowerCase().replace(/\s+/g, " ");

/** Triagens da tabela compartilhada que pertencem ao paciente (mesmo CPF ou mesmo nome). */
export async function listTriagensDoPaciente(patient: Pick<Patient, "name" | "cpf">): Promise<TriagemMulti[]> {
  const supabase = requireSupabase();
  const { slug, password } = requireCompanyCredentials();
  const { data, error } = await supabase.rpc("list_triagens", {
    p_slug: slug,
    p_password: password,
  });
  if (error) throw error;
  const rows = ((data ?? []) as TriagemMultiRow[]).map(mapTriagemMulti);
  const cpf = onlyDigits(patient.cpf);
  const nome = normName(patient.name);
  return rows
    .filter((t) => (cpf.length >= 11 && onlyDigits(t.cpf) === cpf) || (nome !== "" && normName(t.nome) === nome))
    .sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}

export async function upsertTriagemMulti(id: number | null, payload: TriagemMultiInput): Promise<TriagemMulti> {
  const supabase = requireSupabase();
  const { slug, password } = requireCompanyCredentials();
  const { data, error } = await supabase.rpc("upsert_triagem", {
    p_slug: slug,
    p_password: password,
    p_id: id,
    p_payload: payload,
  ...
  return mapTriagemMulti(row as TriagemMultiRow);
}

export type AutolinkTriagemResult = {
  linkedOnly: boolean;
...
/** Vincula a triagem ao paciente (scores + fila). Best-effort: nunca bloqueia o salvamento. */
export async function autolinkTriagemMulti(triagemId: number): Promise<AutolinkTriagemResult | null> {
  try {
    const supabase = requireSupabase();
    const { slug, password } = requireCompanyCredentials();
    const { data, error } = await supabase.rpc("autolink_triagem", {
      p_slug: slug,
      p_password: password,
      p_triagem_id: triagemId,
    });
    if (error) return null;
    return (data as AutolinkTriagemResult) ?? null;
  } catch {
    return null;
  }
}
