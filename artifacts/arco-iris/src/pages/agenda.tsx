...
function expandRecurrence<T extends { date: string; time: string; patientId: number; recurrenceGroupId?: string | null; status: string; frequency?: string | null }>(
  ...
  for (const [, gApts] of groups) {
    ...
    // Recorrência excluída pela Administração: nada de projetar de volta as
    // ocorrências apagadas da data do corte em diante.
...
const NEON: Record<string, React.CSSProperties> = {
  ...
  violet: {
    ...
    color: "#c4b5fd",
    boxShadow: "0 0 14px rgba(139,92,246,0.55), inset 0 0 8px rgba(139,92,246,0.12)",
...
export default function Agenda() {
  ...
  // como parceiro. Sem isso o horário aparece livre na grade de quem foi
  // convidado sempre que a linha própria dele não existe (ou a série divergiu).
  ...
  useEffect(() => {
    ...
    if (!canView || !selectedProfId || !professionals.length || !range) return;
    fetchMultiPartners(range.from, range.to);
  }, [professionals, selectedProfId, canView]);
  ...
  // e não tem linha própria (série do parceiro criada sem recorrência, renovada
  // depois, etc.). Sem isso o horário aparecia livre nesta grade.
            ...
            <div className="w-full max-w-sm rounded-2xl overflow-hidden shadow-2xl" style={{ background: "rgba(8,0,14,0.97)", border: "1px solid rgba(139,92,246,0.4)" }} onClick={e => e.stopPropagation()}>
              <div className="px-6 py-5">
                ...
                <div className="flex gap-3">
                  ...
                  <Button variant="outline" disabled={undoMultiSending} className="flex-1 border-white/10 text-white/60 hover:text-white hover:bg-white/5" onClick={() => setUndoMultiApt(null)}>
                    Cancelar
                  </Button>
                </div>
              </div>
